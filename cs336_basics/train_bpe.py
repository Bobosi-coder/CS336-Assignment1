...
def init_vocab(special_tokens : list[str] | None = None) -> tuple[dict[int, bytes], set[int]]:
    """
...
def pre_tokenize(
    ...
    with open(input_path, 'rb') as f:
        chunk_boundaries = find_chunk_boundaries(f, num_workers, special_token.encode('utf-8'))
...
def bpe_state(words_count : dict[tuple[bytes,...], int]) -> tuple[
    ...
    words = []
    pair_counts = Counter()
    pair_to_word_ids = {} # 倒排索引， (bytes, bytes) -> (words_idx_1, words_idx_2, ...)
    ...
    **kwargs,
    ...
    for pos in tqdm(range(loop_times), desc="Training process"):
        ...
        for idx in words_idxs:
            word_entry = words[idx] # word_entry 是每一条 ([word_byte_1, word_byte_2, ...], word出现次数)
            ...
            while pos < len(word_bytes):
                if (
                    pos < len(word_bytes) - 1 and 
                    word_bytes[pos] == max_pair[0] and 
                    word_bytes[pos + 1] == max_pair[1]):
                    new_word_bytes.append(max_pair[0] + max_pair[1])
                    pos += 2
                else:
                    new_word_bytes.append(word_bytes[pos])
                    pos += 1
            words[idx] = (new_word_bytes, value) # 更换 words 表中的单词 bytes， 将新的pair合并到一起
